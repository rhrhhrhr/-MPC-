...
def uk_a_b_build(u_inf, u_sup, n: int):
    ...
    for i in range(0, n):
        uk_a[4 * i:4 * (i + 1), 2 * i:2 * (i + 1)] = np.mat([[1, 0], [-1, 0], [0, 1], [0, -1]])
...
def mpc_optimization(sys_para, X_bou, U_bou, x_ini, xf):
    ...
    cost_fn = cost_fn + X[:, sys_para['n']].T @ P @ X[:, sys_para['n']]  # 还应加上终端代价函数
    ...
    for i in range(0, xf_num):
        ...
        ubg[n_states * (sys_para['n'] + 1) + i] = float(xf['b'][0, i])  # x(N)在终端区域内
...
if __name__ == "__main__":
    ...
    sim_result = mpc_optimization(system_parameter, X_bounds, U_bounds, x_initial, Xf)
